...
function Modal() {
  const { modalOpen, modalContent, switchModal } = useAppContext();
              ...
              >
                <div className="relative inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                  ...
                  <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                    {!modalContent.noConfirm && (
                      <button
                        type="button"
                        className={`
                      ${
                        modalContent.type === "danger"
                          ? "w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-red-600 text-base font-medium text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-700 sm:ml-3 sm:w-auto sm:text-sm"
                    ...
                    <button
                      ...
                      onClick={() => switchModal(false, null)}
                      ref={cancelButtonRef}
                    >
                      Annuler
                    </button>
