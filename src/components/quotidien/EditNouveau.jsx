...
const validationSchema = Yup.object().shape({
  ...
  encadreur: Yup.mixed().required("Ce champ est requis."),
...
function EditNouveau({ membreId, currentMembre }) {
  ...
  const handleSubmit = (values) => {
    ...
    dispatch(editMembre(membreData))
      ...
      .then(() => {
        let timelineData;
        if (currentMembre.encadreur !== values.encadreur) {
          timelineData = {
            action: `Ce membre est desormais suivi par ${values.encadreur}.`,
            membre: membreId,
          };
        } else if (values.marie !== currentMembre.marie) {
          timelineData = {
            action: `Ce membre est desormais ${
              values.marie === true ? "marié(e)" : "célibataire"
            }.`,
            membre: membreId,
          };
        } else if (values.baptise !== currentMembre.baptise) {
          timelineData = {
            action: `Ce membre est desormais ${
              values.baptise === true ? "baptisé(e)" : "non baptisé(e)"
            }.`,
            membre: membreId,
          };
        }
        ...
        setNotificationContent({
          type: "success",
          title: "Succès",
          description: "Le nouveau a bien été modifié.",
        });
    ...
    <div className="p-4">
      <Form
        initialValues={{
          ...
          encadreur: currentMembre.encadreur,
        }}
      ...
      >
        ...
        />
        <FormField name={"encadreur"} type={"text"} label={"Encadreur"} />
